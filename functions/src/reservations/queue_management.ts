...
interface QueueReservation {
  ...
  serviceId?: string;
  serviceName?: string;
  preferredDate: admin.firestore.Timestamp;
  preferredHour: number; // 0-23
  attendees: AttendeeModel[];
  queuePosition: number;
  estimatedEntryTime?: admin.firestore.Timestamp;
...
interface QueueSettings {
  ...
  processingRate: number; // minutes per person
  isActive: boolean;
...
export const joinQueue = functions.https.onCall(async (data, context) => {
  ...
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'You must be logged in to join a queue'
    );
  }
  ...
  if (!providerId || !governorateId || !preferredDate || preferredHour === undefined) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Missing required fields to join queue'
    );
  }
  ...
  try {
    ...
    const queueSettingsRef = db.collection('queueSettings').doc(
      `${providerId}_${governorateId}_${serviceId || 'general'}_${dateObj.toISOString().slice(0,10)}_${preferredHour}`
    );
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      if (!queueSettingsDoc.exists) {
        ...
        queueSettings = queueSettingsDoc.data() as QueueSettings;
        
        // Check if queue is active
        if (!queueSettings.isActive) {
          throw new functions.https.HttpsError(
            'failed-precondition',
            'This queue is no longer accepting new reservations'
          );
        }
      }
      
      // Check if user is already in queue for this time slot
      const existingQueueQuery = await transaction.get(
        db.collection('queueReservations')
          .where('userId', '==', userId)
      ...
      const estimatedEntryTime = new admin.firestore.Timestamp(
        now.seconds + (estimatedWaitMinutes * 60),
        now.nanoseconds
      );
      ...
      const queueReservation: QueueReservation = {
...
export const checkQueueStatus = functions.https.onCall(async (data, context) => {
  // Check if user is authenticated
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'You must be logged in to check queue status'
    );
  }
  ...
  try {
    ...
    if (!queueReservationDoc.exists) {
      throw new functions.https.HttpsError(
        'not-found',
        'Queue reservation not found'
      );
    }
    
    const queueReservation = queueReservationDoc.data() as QueueReservation;
    
    // Check if this reservation belongs to the user
    if (queueReservation.userId !== userId) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'You do not have permission to check this queue reservation'
      );
    }
    ...
    if (!queueSettingsDoc.exists) {
      throw new functions.https.HttpsError(
        'not-found',
        'Queue settings not found'
      );
    }
    
    const queueSettings = queueSettingsDoc.data() as QueueSettings;
    ...
    throw new functions.https.HttpsError(
      'internal',
      error instanceof Error ? error.message : 'Failed to check queue status'
    );
  }
});
...
export const leaveQueue = functions.https.onCall(async (data, context) => {
  ...
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'You must be logged in to leave a queue'
    );
  }
  ...
  try {
    ...
    if (queueReservation.userId !== userId) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'You do not have permission to cancel this queue reservation'
      );
    }
...
export const processQueues = functions.pubsub
  ...
  .onRun(async (context) => {
    try {
      ...
      for (const queueDoc of activeQueuesQuery.docs) {
        const queueSettings = queueDoc.data() as QueueSettings;
        ...
        for (const entryDoc of queueEntriesQuery.docs) {
          const queueReservation = entryDoc.data() as QueueReservation;
...
export const completeQueueEntry = functions.https.onCall(async (data, context) => {
  // Check if user is authenticated
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'You must be logged in to complete a queue entry'
    );
  }
  ...
  try {
    ...
    const isProvider = userData?.providerId === queueReservation.providerId;
    
    if (!isAdmin && !isProvider) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'You do not have permission to complete this queue entry'
      );
    }
    ...
    const playerId = customerDoc.data()?.oneSignalPlayerId;
    ...
    throw new functions.https.HttpsError(
      'internal',
      error instanceof Error ? error.message : 'Failed to complete queue entry'
    );
  }
});

// Function to mark no-show (called by service provider)
export const markNoShow = functions.https.onCall(async (data, context) => {
  // Check if user is authenticated
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'You must be logged in to mark a no-show'
    );
  }
  ...
  try {
    ...
    if (!isAdmin && !isProvider) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'You do not have permission to mark this as no-show'
      );
    }
...
export const autoEnrollReservationsToQueue = functions.pubsub
  ...
  .onRun(async (context) => {
    try {
      ...
      for (const reservationDoc of upcomingReservationsQuery.docs) {
        const reservation = reservationDoc.data() as ReservationModel;
        
        // Check if reservation date/time is within 30 mins from now
        const reservationTime = new Date(
          reservation.date.toDate().getFullYear(),
          reservation.date.toDate().getMonth(),
          reservation.date.toDate().getDate(),
        ...
        if (timeDiffInMinutes > 30) continue;
...
export const getUserQueueHistory = functions.https.onCall(async (data, context) => {
  // Check if user is authenticated
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'You must be logged in to view queue history'
    );
  }
  ...
  try {
    ...
    for (const doc of activeQueuesQuery.docs) {
      const queueData = doc.data() as QueueReservation;
