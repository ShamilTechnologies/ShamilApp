  ...
  Future<void> _checkAuthAndNavigate() async {
     ...
     try {
        ...
        final state = await authBloc.stream.firstWhere(
           (s) => s is AuthInitial || s is LoginSuccessState || s is AwaitingVerificationState || s is AuthErrorState,
           orElse: () => const AuthInitial() // Default if stream closes unexpectedly
        );
        ...
        final bool onboardingShown = AppLocalStorage.getData(key: AppLocalStorage.isOnboardingShown) ?? false;

        print("SplashView Navigating: OnboardingShown=$onboardingShown, AuthState=${state.runtimeType}");
        ...
        if (!onboardingShown) {
           ...
           targetScreen = const MainNavigationView(); // Already logged in and verified
        } else if (state is AwaitingVerificationState) {
           targetScreen = const LoginView(); // Go to login, show "verify email" message there
        } else { // AuthInitial or AuthError
...
class SquareClipper extends CustomClipper<Path> {
  ...
  @override Path getClip(Size size) {
    ...
    return Path()..addRect(Rect.fromCenter(center: center, width: effectiveSide, height: effectiveSide));
  }
  @override bool shouldReclip(SquareClipper oldClipper) => oldClipper.side != side;
}
