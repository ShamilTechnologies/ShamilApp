...
import 'package:shamil_mobile_app/core/utils/text_style.dart'; // Keep for potential use in helpers
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void initState() {
    ...
    // Optionally load friends data here too
  ...
  Future<void> _pickAndUpdateProfilePicture() async {
    ...
    try {
       ...
       } else if (pickedImage == null) { showGlobalSnackBar(context, "Image selection cancelled."); }
    } catch (e) {
  ...
  Future<void> _handleLogout() async {
     ...
     if (confirm == true) { try { context.read<AuthBloc>().add(const LogoutEvent()); } catch (e) { print("Error dispatching logout event: $e"); if (mounted) { ScaffoldMessenger.of(context).showSnackBar( SnackBar(content: Text("Logout failed: $e")), ); } } }
   }
  // --- End Logout Logic ---
          ...
          listeners: [
             ...
             BlocListener<AuthBloc, AuthState>(
               listener: (context, state) {
                 ...
                 if (state is LoginSuccessState && _isUpdatingPicture) { showGlobalSnackBar(context, "Profile picture updated!"); }
                 else if (state is AuthErrorState && _isUpdatingPicture) { showGlobalSnackBar(context, "Update failed: ${state.message}", isError: true); }
                 else if (state is PasswordResetEmailSentState) { showGlobalSnackBar(context, "Password reset email sent."); }
               },
             ),
             // Listener for SocialBloc side effects
            ...
            builder: (context, authState) {
              ...
              if (authState is LoginSuccessState) {
                 ...
                 return ProfileContent(
                    ...
                    onUpdatePicture: _pickAndUpdateProfilePicture, // Pass callback down
  ...
  /// Widget to display error message (kept here as it needs context for retry)
