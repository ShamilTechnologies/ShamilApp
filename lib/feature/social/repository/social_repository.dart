...
/// Repository for managing social connections like friends and family members
abstract class SocialRepository {
  ...
  Future<List<AuthModel>> getCurrentUserFriends();

  /// Get family members for the specified user
  ...
  Future<Map<String, dynamic>> removeFamilyMember({
    required String currentUserId,
    required String
        memberDocId, // Doc ID in current user's familyMembers subcollection
  ...
  Future<Map<String, dynamic>> acceptFamilyRequest({
    ...
    required String
        requesterRelationship, // Relationship requester assigned to current user
  });
  ...
  Future<List<UserSearchResultWithStatus>> searchUsersByNameOrUsername(
      String currentUserId, String query);
  Future<AuthModel?> searchUserByNationalId(
      String nationalId, String currentUserId);
}
...
class FirebaseSocialRepository implements SocialRepository {
  ...
  CollectionReference _usersCollectionRef() =>
      _firestore.collection('endUsers'); // Standardized collection name

  Future<Map<String, dynamic>> _callFunction(
      String functionName, Map<String, dynamic> payload) async {
    debugPrint(
        "FirebaseSocialRepository: Calling '$functionName' with payload: $payload");
    try {
      ...
      debugPrint(
          "FirebaseSocialRepository: FirebaseFunctionsException for $functionName - Code: ${e.code}, Msg: ${e.message}, Details: ${e.details}");
      return {
        'success': false,
      ...
      debugPrint(
          "FirebaseSocialRepository: Generic error for $functionName: $e");
      return {
        'success': false,
  ...
  @override
  Future<List<AuthModel>> getCurrentUserFriends() async {
    try {
      // Mock implementation - in a real app, fetch from Firestore
      return [
        AuthModel(
          uid: 'friend1',
          name: 'John Smith',
  ...
  Future<Map<String, dynamic>> addOrRequestFamilyMember({
    ...
    final payload = {
      'memberData':
          memberData, // Contains name, relationship, phone, email, etc.
      'currentUserData': {
        // For creating the reciprocal entry if a request is sent
  ...
  Future<Map<String, dynamic>> removeFamilyMember({
    required String currentUserId,
    required String
        memberDocId, // Doc ID of the family member in current user's subcollection
    String?
        linkedUserId, // UID of the linked user if applicable (for reciprocal removal)
  }) async {
    final payload = {
  ...
  Future<List<FamilyMember>> fetchFamilyMembers(String userId) async {
    final snapshot = await _usersCollectionRef()
        .doc(userId)
  ...
  Future<List<FamilyRequest>> fetchIncomingFamilyRequests(String userId) async {
    ...
    return snapshot.docs
        .map((doc) => FamilyRequest.fromFirestore(doc))
        .toList();
  }
  ...
  Future<List<UserSearchResultWithStatus>> searchUsersByNameOrUsername(
    ...
    final nameSnapshot = await _usersCollectionRef()
        ...
        .where('name_lowercase',
            isLessThanOrEqualTo:
                '$lowerQuery\uf8ff') // Standard pattern for prefix match
        .limit(10)
        .get();
    ...
    for (var userModel in combinedResults.values) {
      ...
      try {
        final friendDoc = await _usersCollectionRef()
            .doc(currentUserId)
            .collection('friends')
            .doc(userModel.uid)
            .get();
      ...
      resultsWithStatus.add(
          UserSearchResultWithStatus(user: userModel, status: currentStatus));
    }
  ...
  @override
  Future<AuthModel?> searchUserByNationalId(
      String nationalId, String currentUserId) async {
    ...
    if (querySnapshot.docs.isNotEmpty) {
      ...
      if (doc.id != currentUserId) {
        // Ensure not to return the current user
