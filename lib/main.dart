...
Future<void> main() async {
  ...
  } catch (e) { print("Error loading .env file: $e"); }
  ...
  } catch (e) { print("Error activating Firebase App Check: $e"); }
...
class _MainAppState extends State<MainApp> {
   ...
   Future<void> _setupFcmListenersAndPermissions() async {
      ...
      if (settings.authorizationStatus == AuthorizationStatus.authorized ||
         ...
         print('Notification Permission Granted (or provisional). Setting up listeners and token handling.');
         ...
         FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) {
            print('Message clicked (background)! Navigating based on data: ${message.data}');
   ...
   Future<void> _handleTerminatedNotification() async {
      ...
      if (initialMessage != null) {
         print('Message clicked (terminated)! Navigating based on data: ${initialMessage.data}');
   ...
   void _handleNotificationTap(BuildContext navContext, Map<String, dynamic> data) {
      print("Handling notification tap with data: $data");
      final String? type = data['type'] as String?; // Get notification type from data
         ...
         try {
            Navigator.of(navContext).push(MaterialPageRoute(
               builder: (_) => BlocProvider.value(
                  value: BlocProvider.of<SocialBloc>(navContext), // Provide existing SocialBloc
             ...
             print("Error navigating from notification tap (maybe Bloc not found?): $e");
   ...
   Future<void> _updateTokenInFirestore(String? token) async {
      ...
      try {
         ...
         await FirebaseFirestore.instance.collection('endUsers').doc(user.uid).set({
   ...
   Future<void> _removeTokenFromFirestore(String? token, String? userId) async {
      ...
      try {
         await FirebaseFirestore.instance.collection('endUsers').doc(userId).update({
      ...
      builder: (context, child) {
         return BlocListener<AuthBloc, AuthState>(
            listener: (context, state) {
               ...
               print("Global Auth Listener: State=${state.runtimeType}, CurrentRoute=$currentRoute");

               if (state is AuthInitial && currentRoute != null && currentRoute != '/') {
                  // If user becomes unauthenticated (logged out) and not on splash/login, force to login
                  print("Global Auth Listener: Detected AuthInitial, navigating to Login.");
                  // Get token *before* navigating away / potentially losing user context
                  FirebaseMessaging.instance.getToken().then((currentToken) {
                     // We need the UID from the *previous* state before AuthInitial
                     // This is complex. AuthBloc's logout handler should ideally store
                     // the UID temporarily or trigger a server-side function for removal.
                     // For now, we just delete the local token instance.
                     if (currentToken != null) {
                        FirebaseMessaging.instance.deleteToken();
                        print("FCM Token deleted from device on logout.");
                        // Attempt removal from Firestore (might fail if UID isn't available)
                        // String? lastUserId = context.read<AuthBloc>().lastUserId; // Example if Bloc stored it
                        // if(lastUserId != null) _removeTokenFromFirestore(currentToken, lastUserId);
                     }
                  });
